# Meld — registro de decisiones (histórico)

> Archivo de contexto histórico: qué se probó, qué se descartó, quién lo pidió y
> cuándo. **No es la fuente vigente** — puede estar desactualizado. Las reglas
> actuales viven en `CLAUDE.md`; consultar esto solo cuando haga falta entender
> el porqué de una decisión antes de revertirla. Snapshot del `CLAUDE.md` del
> 2026-09-24, antes del refactor que lo dejó solo con reglas vigentes.
...
/src
  ...
  /components               → design system compartido, agrupado por área
    ...
    ConfirmDialog.tsx         → modal de confirmación propio del design system
                              (reemplaza `Alert.alert` nativo — usado por
                              `CategoriesManagerSheet` y `TrialEndedDialog`)
    ...
    calendar/CalendarViewSwitch.tsx → switch de 3 íconos (Mes/Semana/Año),
                              ...
                              "Calendar Screen" (que usa texto, no
...
- **`dayItemsStore`**: `reload()` ahora, además de `listByDate`, trae
  ...
  para `selectedDateKey` y recalcula progress/streak, con snackbar de
  Deshacer (ver "Marcar CUALQUIER hábito" más abajo). El contador
...
- Eliminar un ítem borra directo y muestra la fila de Deshacer (ver "Eliminar
  un ítem (swipe)"). Cuando una acción sí necesita confirmar antes (p. ej.
  borrar una categoría), usar `ConfirmDialog` (`components/ConfirmDialog.tsx`),
  nunca `Alert.alert` nativo — respeta paleta/tipografía en claro y oscuro.
...
- **Arrastrar para poner hora** (`components/TimeDragOverlay.tsx` +
  ...
  confirmar/cancelar es `TimeConfirmRow`, la card flotante que `FloatingBar`
  dibuja encima de la barra mientras `useTimeDragStore().active` es true (misma
  card que `UndoRow`/`topRow`) — no crear un overlay independiente de
  `FloatingBar`, quedaría superpuesto con el menú principal. Todo esto es fiel a un video de
...
- **Eliminar un ítem (swipe)**: en Today, Inbox y la agenda Semana de Calendar,
  ...
  design system y hoy lo usan `CategoriesManagerSheet` (borrar categoría) y
  `TrialEndedDialog`. Si una fila completada (`completed: true`) usa
...
- **Completar una Task (desaparece de Today)**: tocar el checkbox de una Task
  ...
  explícito: "ten en cuenta ese registro para la vista de Calendario"). Los
  hábitos siguen el mismo patrón con su propio toggle (`habitIcon`, ver
  "Hábitos recurrentes y tracking real").
...
- **Próximos recordatorios** (botón "más" `more-horiz` de `TopAppBar` +
...
- **Filtro de Calendario** (`store/calendarFilterStore.ts` +
  ...
  **`CalendarFilterBar.tsx` (la pill flotante que vivía anclada arriba de
  ...
  más abajo), a la agenda semanal y al heatmap de Año (filtrar por un hábito
  agrega solo ese hábito; ver "Heatmap real" en "Hábitos recurrentes"). El ícono
...
- **Grid de Mes: puntos de color + miniatura de Moment por día**
  ...
  de referencia "Calendar Screen") — reemplazó las etiquetas mini de texto (títulos
  ...
  (`typeColors[item.type]`, tope `MAX_DOTS = 3`). Si un día
...
- **3 vistas de Calendario** (`SegmentedToggle`, orden Mes/Semana/Año), **Mes es la vista inicial al entrar a Calendar** (`useState<CalendarView>('month')` en `CalendarScreen.tsx`, cambiado 2026-08-26 a pedido explícito — antes abría en Año):
...
- **Estados vacíos** (`components/EmptyState.tsx`): Today (sin ítems agendados
  ...
  de referencia "Empty State / No Tasks Today"; Today abre Quick Add, Inbox enfoca el
...
- **Tarjeta "Today" debajo del grid de Mes** (`components/calendar/
  ...
  de referencia "Calendar Screen"): dentro del `ScrollView` de la vista Mes, justo
...
- **Header de Calendario rediseñado + navegación entre meses** (agregado
  2026-08-26, fiel al mock de referencia "Calendar Screen", a pedido explícito tras
...
semanal). Los 4 comparten paleta/lenguaje visual (mocks previos,
uno por tipo, pedido explícito antes de tocar
...
(mock previo, pedido explícito antes de tocar código): pill
...
celdas bastante más grandes que el mock de referencia — cada
...
- **Android, real y probado en dispositivo**: `react-native-android-widget`
  ...
  `src/widget/refreshWidgets.ts` → `refreshWidgets()` es el punto de
